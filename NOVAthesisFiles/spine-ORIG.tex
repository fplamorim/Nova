...
\spine(box,color,bg):=?{none}
\spine(box,color,text):=?{black}
...
\ifoptionequal{/novathesis/spine/layout}{none}{}{%
  ...
  \ifoptionequal{/novathesis/spine/layout}{trim}{%
    \newstocksize{layoutsize={\the\stockheight,\the\@spinewidth},margin=0pt}%
  }{\ifoptionequal{/novathesis/spine/layout}{full}{%
    \newstocksize{layoutsize={\the\stockheight,\the\stockwidth},margin=0pt}%
  }{%
 ...
 \ifoptionequal{/novathesis/spine/layout}{trim}
  ...
  \begin{tikzpicture}[remember picture, overlay,yscale=-1,yshift=-10.75pt+\@tempdima]%
    ...
    \@for\myi:=\expanded{\thespine(order)}\do{%
      \datamatch{\@boxcolor}{spine}({box,\myi,color,bg},{box,color,bg})%
      \node[rectangle,
            inner sep = 0pt,
            anchor = north west,
...
\newcommand{\printspinebox}[2]{%
  ...
  \IfStrEq{#1}{logo}{%
    ...
    \scalebox{\@fbratio}{%
      \datamatchtf{\arg}{spine}({box,color,text},{text,color}){\color{\arg}}{}%
